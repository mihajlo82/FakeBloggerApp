...
export default function Login({setIsLogged}) {
 ...
 const data = React.useContext(MyBloggerContext);
 const {currUser} = data;
 const [currentUser, setCurrentUser] = currUser;
 
 const {handleSubmit} = useLogin({username, password, setCurrentUser, setIsLogged})
        ...
        <Grid item xs={12} sm={8} md={5} component={Paper} elevation={6} square>
          <Box sx={{  my: 8,  mx: 4, display: 'flex', flexDirection: 'column', alignItems: 'center' }} >
            ...
            <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 1 }}>
              <TextField  value={username} onChange={(e)=>setUsername(e.target.value)} margin="normal" required  fullWidth id="email" label="Username" name="username" autoComplete="username" autoFocus />
              ...
              <Button onClick={handleSubmit} type="button" fullWidth variant="contained" sx={{ mt: 3, mb: 2 }} > Sign In </Button>
            </Box>
          </Box>
        </Grid>
      </Grid>
